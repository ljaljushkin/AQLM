...
command_template='lm_eval --model=hf --model_args=pretrained=HuggingFaceTB/SmolLM-1.7B-Instruct,trust_remote_code=True,nncf_ckpt_dir=${nncf_ckpt_dir},device_map=auto,parallelize=True,dtype=float32 --tasks=wikitext'
...
# run_commands "FQ_4bit_no_embed_svd_rank256_g64_float32" "${rank8_names[@]}"
 run_commands "FQ_4bit_no_embed_svd_rank256_g64_float32_rand_quant" "${rank8_names[@]}"
